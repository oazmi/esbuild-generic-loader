...
import { DEBUG, escapeLiteralStringForRegex, json_stringify, zipArrays, zipIteratorsMapperFactory } from "./deps.js";
const defaultGenericLoaderConfig = { meta: true }, escape_regex_for_string_raw = /(\$)|(\`)|(\<\/script\>)/g, imports_beginning_marker = "globalThis.start_of_imports()", imports_ending_marker = "globalThis.end_of_imports()", import_statements_block_regex = new RegExp(escapeLiteralStringForRegex(imports_beginning_marker)
    + "[\,\;]*" // if esbuild minification is enabled, then either a ";" or a "," delimiter will be placed between statements instead of a new line.
    + `(?<importStatements>.*?)`
    + escapeLiteralStringForRegex(imports_ending_marker), "gs"), import_statement_regex = new RegExp("await\\s+import\\(\\s*\"(?<importPath>.*?)\"\\s*\\)[\,\;]*", "g"), deps_list_to_js_fn = zipIteratorsMapperFactory(([import_key, import_path]) => {
    return `
...
export class GenericLoader {
    ...
    */
     ...
     * ```
    */
    async parseToJs(raw_content) {
        const { content, importKeys, importPaths } = await this.extractDeps(raw_content), deps_js_string = [...deps_list_to_js_fn(importKeys, importPaths)].join(""), content_export_js = await this.contentExportJs(content);
        if (DEBUG.META) {
            const meta_imports = this.meta.imports;
            for (const [key, path] of zipArrays(importKeys, importPaths)) {
...
${content_export_js}`;
    ...
    */
    async unparseFromJs(js_content) {
        const importPaths = [];
        const js_content_without_imports = js_content.replaceAll(import_statements_block_regex, (_full_match, ...args) => {
            // here, `_full_match` is:
            // `${imports_beginning_marker}\n${deps_js_string}\n${imports_ending_marker}`
            const [named_groups, _full_string, _offset, ..._unused_groups] = args.toReversed(), marked_import_statements = named_groups.importStatements;
            ...
            const ordered_import_key_statements = marked_import_statements.replaceAll(import_statement_regex, (_full_match, ...args) => {
                ...
                const [named_groups, _full_string, _offset, ..._unused_groups] = args.toReversed(), bundled_import_path = named_groups.importPath;
        ...
        const js_blob = new Blob([js_content_without_imports], { type: "text/javascript" }), js_blob_url = URL.createObjectURL(js_blob), 
        ...
        { content, importKeys } = await import(js_blob_url), { meta: { imports: metaImports }, config: { meta: metaEnabled } } = this, number_of_imports = importKeys.length;
        if (DEBUG.ASSERT && (number_of_imports !== importPaths.length
        ...
        if (DEBUG.META && metaEnabled) {
            for (const [key, path, import_entry] of zipArrays(importKeys, importPaths, metaImports)) {
